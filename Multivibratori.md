...
- **Monostabili**: sono circuiti con _un solo stato stabile_. Quando vengono sollecitati da un _trigger_ esterno passano per un certo tempo definito $T$ in uno stato _astabile_, per poi tornare alla stabilità. Un esempio classico di _multivibratore monostabile_ è il _timer_
...
Supponiamo quindi di avere come condizione iniziale che nel tempo $t = 0$ la tensione $V_1$ viene commutata a $V_{DD}$ e che gli inverter siano tutti uguali con tempo di ritardo $\tau$.
...
Quello che otteniamo è quindi un segnale instabile di periodo $6 \tau$.

In generale, con $n = 2k + 1$ inverter, il periodo sarà di $2n \tau$, e di conseguenza la frequenza sarà $f = \frac{1}{2n\tau}$
...
$$
\begin{align*}
	T &= 2 \cdot \sum_{i=1}^n{\tau_i} \\
	D = \frac{\sum_{i=1}^n{\tau_i}}{T} = \frac{1}{2}
\end{align*}
$$
...
Al suo interno è composto come sulla destra: tre resistenze uguali in serie collegate tra il piedino `8` ($V_{CC} \in [4 \div 15]$ $V$) e il piedino `1` ($0$).
...
- Attraverso una resistenza $R$ è portato all'ingresso di un _transistore bipolare_ $Q_1$ che ha come _Collettore_ $D$ il piedino `7`, mentre l'emettitore è collegato a _ground_.
...
Per cpaire come utilizziamo il `NE 555` dovviamo analizzare il seguente circuito:
<div class="grid2">
<div class="">
...
In condizioni ideali, i due amplificatori operazionali non assorbono corrente, perciò $i_{TH} = i_{TR} = i = 0$.
...
In questo circuito il condensatore quindi tende a caricarsi, poiché ha una tensioen $V_C < V_{CC}$.
...
<div class="">
TODO: circuito semplificato set
<img class="80" src="./images">
</div>
</div>

Quando la tensione sale oltre $V_C > \frac{2}{3}V_{CC}$, succede che `S = 0` e `R = 1`, che provoca la **commutazione dell'uscita**, entrado di fatto nella **fase di reset**.
...
$$
\begin{align*}
	V_C(t = t_1) = V_{COM} &= V_f + (V_i - V_f)e^{T_1 \over \tau} \\
	e^{T_1 \over \tau} &= \frac{V_i - V_f}{V_{COM} - V_f} \\
	T_1 &= \tau \cdot \ln{\frac{V_i - V_f}{V_{COM} - V_f}}
\end{align*}
$$

Se la applichiamo alla fase di _set_:
$$
\begin{align*}
...
\large
\begin{align*}
	T_1 &= C \cdot (R_A + R_B) \cdot \ln{\frac{\frac{1}{3}V_{CC} - V_{CC}}{\frac{2}{3}V_{CC} - V_{CC}}} \\
		&= C \cdot (R_A + R_B) \cdot \ln{\frac{-\frac{2}{3}V_{CC}}{-\frac{1}{3}V_{CC}}} \\
		&= \boxed{C \cdot (R_A + R_B) \cdot \ln{2}} \\
\end{align*}
$$

Se invece l'applichiamo alla fase di _reset_:
$$
\begin{align*}
...
\large
\begin{align*}
	T_2 &= C \cdot R_B \cdot \ln{\frac{\frac{2}{3}V_{CC} - 0}{\frac{1}{3}V_{CC} - 0}} \\
...
Questo circuito ha quindi _Duty Cycle_ necessariamente minore di $\frac{1}{2}$, ma comunque, con scelte opportune di $R_A$ e $R_B$, può essere utilizzato per avere un Duty Cycle.

Per verificare la commutazione è necessario che siano valide le cascate di disuguaglianze:
$$
\begin{cases}
